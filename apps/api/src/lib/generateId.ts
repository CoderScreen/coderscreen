...
export const idString = <T extends keyof Entities>(entity: T): z.ZodType<Id<T>, z.ZodTypeDef, string> => {
	return z
		.string()
		.regex(new RegExp(`^${Entities[entity]}_.+`), `Invalid ID: must start with ${Entities[entity]}_`)
		.transform((id) => id as Id<T>);
};
