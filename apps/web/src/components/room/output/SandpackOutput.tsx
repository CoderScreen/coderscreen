...
export const SandpackOutput = () => {
	...
	const { sandpackClient, sandpackLoading, initializeSandpackClient } =
		useSandpackContext();

	// Files, environment and dependencies
	const content: SandboxSetup = useMemo(
		() => ({
			files: {
				"/package.json": {
					code: JSON.stringify({
...
`,
	...
	useEffect(() => {
		...
		const loadClient = async () => {
			...
			await initializeSandpackClient(iframeRef.current, content, options);
		};

		loadClient();
	}, [content, initializeSandpackClient]);
	...
	return (
		<div className="h-full w-full bg-white flex flex-col relative">
			...
			<div className="flex items-center justify-end absolute top-2 right-2 z-10">
				<Tooltip content="Reset the sandbox" showArrow>
					<Button
						variant="icon"
						onClick={() => {
				...
				{logs.length === 0 ? (
					...
					logs.map((log, index) => (
						<div key={`${log.id}-${index}`} className="mb-1">
							<span className={`${getLogColor(log.method)}`}>
