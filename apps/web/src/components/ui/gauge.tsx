 ...
 * @param reverseValue - Option to reverse the value of the gauge. So that 100% is red, 0% is green.
 */
export function Gauge({
  ...
  // Stroke dasharrays
  const primaryStrokeDasharray = calculatePrimaryStrokeDasharray(
    strokePercent,
    offsetFactor,
    gapPercent,
    circumference
  ...
  // Transforms
  const primaryTransform = calculatePrimaryTransform(
    strokePercent,
    offsetFactor,
    gapPercent,
    isAscendingVariant
  ...
  // Stroke colors
  const primaryStroke = calculatePrimaryStroke(
    primary ?? '',
    reverseValue ? 100 - strokePercent : strokePercent
  );
  const secondaryStroke = calculateSecondaryStroke(
    secondary ?? '',
    reverseValue ? 100 - strokePercent : strokePercent
  );

  // Opacities
  const primaryOpacity = calculatePrimaryOpacity(
    offsetFactor,
    strokePercent,
    gapPercent,
    offsetFactorSecondary
  );
  const secondaryOpacity = calculateSecondaryOpacity(
    offsetFactor,
    strokePercent,
    gapPercent,
    offsetFactorSecondary
  ...
  return (
    <svg
      ...
      width={sizeConfig[size as keyof typeof sizeConfig]?.size || size}
      height={sizeConfig[size as keyof typeof sizeConfig]?.size || size}
      fill='none'
